...
export const redirectToAuth = async (action: 'login' | 'register' = 'login') => {
  try {
    ...
    const authUrl = `${AUTH_BASE_URL}${endpoint}?app_id=${APP_ID}&redirect_uri=${encodeURIComponent(window.location.origin + '/auth/callback')}`;
    
    // Verificar si el sistema de autenticación está disponible
    fetch(AUTH_BASE_URL + '/health', { method: 'HEAD' })
      .then(() => {
        window.location.href = authUrl;
      })
      .catch(() => {
        // Si el sistema de autenticación no está disponible, mostrar error
        throw new Error(`Sistema de ${action === 'register' ? 'registro' : 'autenticación'} no disponible. Intenta nuevamente en unos minutos.`);
      });
